// Gitee Admin API - 用于管理员操作 Gitee 仓库
import adminConfig from '@/config/admin.config.js'

const { owner, repo, branch, apiBase } = adminConfig.gitee

// 从 localStorage 获取 token
function getAccessToken() {
  const token = localStorage.getItem('gitee_access_token')
  if (!token) {
    throw new Error('未配置 Gitee Access Token，请先在管理面板配置')
  }
  console.log('🔑 Using Gitee Token:', token.substring(0, 20) + '...')
...
export async function getRepoFiles() {
  ...
  const url = `${apiBase}/repos/${owner}/${repo}/git/trees/${branch}?recursive=1&access_token=${token}&t=${timestamp}`
...
export async function getFileContent(filepath) {
  ...
  const url = `${apiBase}/repos/${owner}/${repo}/contents/${encodeURIComponent(
    filepath
  )}?access_token=${token}&ref=${branch}&t=${timestamp}`
...
export async function updateFile(filepath, content, sha, message = '更新文章') {
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json()
      console.error('Gitee API 错误详情:', errorData)
      throw new Error(errorData.message || `HTTP ${response.status}`)
    }
...
export async function verifyToken() {
  try {
    const token = getAccessToken()
    const url = `${apiBase}/user?access_token=${token}`
    const response = await fetch(url)
    return response.ok
  } catch (error) {
    return false
